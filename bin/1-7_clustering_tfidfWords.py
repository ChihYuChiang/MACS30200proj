...
#%%
#--Read in data
df = pickle.load(open(r'..\data\process\score_original_tfidfWords.p', 'rb'))
df_scores = df[['group0', 'group1']]
...
# for i, word in enumerate(df['Game Title']):
#     ax.annotate(word, (tsneGames[:, 0][i],tsneGames[:, 1][i]))
plt.xticks(())
plt.yticks(())
plt.savefig(r'..\img\1-7_tfidf_tsne')
plt.show()
plt.close()
...
for i in range(numClusters):
    top = kmScores.query('group_label == @i + 1').sort_values('group_' + str(i + 1))[-20: ]['game_title']
...
plt.savefig(r'..\img\1-7_tfidf_k-mean_PCA_' + str(numClusters))
plt.show()
...
plt.yticks(())
plt.title('Predicted Clusters\n k = {}'.format(numClusters))
plt.savefig(r'..\img\1-7_tfidf_k-mean_tsne_' + str(numClusters))
plt.show()
...
for n_clusters in range_n_clusters:
    ...
    plt.savefig(r'..\img\1-7_tfidf_silhouette_' + str(n_clusters))
    # plt.show()
    plt.close()