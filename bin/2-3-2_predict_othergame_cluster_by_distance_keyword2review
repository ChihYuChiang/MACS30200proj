...
df = pickle.load(open(r'..\data\process\score_new_all_doc2vec.p', 'rb')) #df with distances from the last step
...
core_vec = df.query('Core > 0').filter(regex=('^f_\d+$'))
other_vec = df.query('Core == 0').filter(regex=('^f_\d+$'))
...
model = MultinomialNB()
model.fit((core_vec - np.min(core_vec)), core_groups)
labels = model.predict((other_vec - np.min(other_vec)))
labels.shape
...
df_predicted.to_csv(r'..\data\output\df_predicted_all_doc2vec.csv', index=False, encoding='utf-8')
...
plt.savefig(r'..\img\2-3-2_k-mean_PCA_' + str(numOfCluster))
plt.show()
plt.close()
...
plt.yticks(())
plt.title('Predicted Clusters\n k = {}'.format(numOfCluster))
plt.savefig(r'..\img\2-3-2_k-mean_tsne_' + str(numOfCluster))
plt.show()
plt.close()
